...
//------
		...
		function isDate(d){
			...
			function printMe(id){
				//w= window.open('marshgrad.asp?action=nothing&next=print&gradid='+id);
				if(document.all){
					openpopup('marshgrad.asp?action=nothing&next=print&gradid='+id,'PRINTOUT',100,100,10000,10000,'no','yes')
				}
				if(document.layers){
					openpopup('marshgrad.asp?action=nothing&next=print&gradid='+id,'PRINTOUT',500,600,10,10,'no','yes')
				}
			}
